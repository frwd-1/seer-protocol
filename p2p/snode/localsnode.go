...
const (
	...
	// time needed to wait between two updates to the local SNR
	recordUpdateThrottle = time.Millisecond
)

// LocalNode produces the signed node record of a local node, i.e. a node run in the
// current process. Setting SNR entries via the Set method updates the record. A new version
...
func NewLocalNode(db *DB, key *ecdsa.PrivateKey) *LocalNode {
	ln := &LocalNode{
		...
		db:      db,
		key:     key,
		entries: make(map[string]snr.Entry),
		endpoint4: lnEndpoint{
...
// the last change.
func (ln *LocalNode) Set(e snr.Entry) {
	ln.mu.Lock()
	defer ln.mu.Unlock()
...
func (ln *LocalNode) set(e snr.Entry) {
	val, exists := ln.entries[e.SNRKey()]
	if !exists || !reflect.DeepEqual(val, e) {
		ln.entries[e.SNRKey()] = e
		ln.invalidate()
	}
}

// Delete removes the given entry from the local record.
func (ln *LocalNode) Delete(e snr.Entry) {
	ln.mu.Lock()
	defer ln.mu.Unlock()
...
func (ln *LocalNode) delete(e snr.Entry) {
	_, exists := ln.entries[e.SNRKey()]
	if exists {
		delete(ln.entries, e.SNRKey())
		ln.invalidate()
	}
}
...
func (ln *LocalNode) updateEndpoints() {
	...
	if udp6 != 0 && udp6 != udp4 {
		ln.set(snr.UDP6(udp6))
	} else {
		ln.delete(snr.UDP6(0))
	}
}
