...
func newLookup(ctx context.Context, tab *Table, target snode.ID, q queryFunc) *lookup {
	it := &lookup{
		tab:       tab,
		queryfunc: q,
		asked:     make(map[snode.ID]bool),
		seen:      make(map[snode.ID]bool),
...
func (it *lookupIterator) Node() *snode.Node {
	if len(it.buffer) == 0 {
		return nil
	}
