...
type codecV5 interface {
	...
	Encode(snode.ID, string, v5wire.Packet, *v5wire.Whoareyou) ([]byte, v5wire.Nonce, error)

	// Decode decodes a packet. It returns a *v5wire.Unknown packet if decryption fails.
	// The *snode.Node return value is non-nil when the input contains a handshake response.
	Decode([]byte, string) (snode.ID, *snode.Node, v5wire.Packet, error)
}
...
type UDPv5 struct {
	...
	localNode    *snode.LocalNode
	db           *snode.DB
	log          log.Logger
	clock        mclock.Clock
	validSchemes snr.IdentityScheme
	...
	// state of dispatch
	codec            codecV5
	activeCallByNode map[snode.ID]*callV5
	activeCallByAuth map[v5wire.Nonce]*callV5
	callQueue        map[snode.ID][]*callV5
...
type callV5 struct {
	id   snode.ID
	addr *net.UDPAddr
	node *snode.Node // This is required to perform handshakes.
...
func ListenV5(conn UDPConn, ln *snode.LocalNode, cfg Config) (*UDPv5, error) {
...
func newUDPv5(conn UDPConn, ln *snode.LocalNode, cfg Config) (*UDPv5, error) {
	...
	t := &UDPv5{
		...
		activeCallByNode: make(map[snode.ID]*callV5),
		activeCallByAuth: make(map[v5wire.Nonce]*callV5),
		callQueue:        make(map[snode.ID][]*callV5),
		// shutdown
		closeCtx:       closeCtx,
...
func (t *UDPv5) Ping(n *snode.Node) error {
	_, err := t.ping(n)
	return err
}
...
func (t *UDPv5) Resolve(n *snode.Node) *snode.Node {
	if intable := t.tab.getNode(n.ID()); intable != nil && intable.Seq() > n.Seq() {
		n = intable
	}
	// Try asking directly. This works if the node is still responding on the endpoint we have.
	if resp, err := t.Requestsnr(n); err == nil {
		return resp
	}
...
func (t *UDPv5) AllNodes() []*snode.Node {
	t.tab.mutex.Lock()
	defer t.tab.mutex.Unlock()
	nodes := make([]*snode.Node, 0)
...
// protocol.
func (t *UDPv5) LocalNode() *snode.LocalNode {
	return t.localNode
}
...
func (t *UDPv5) TalkRequest(n *snode.Node, protocol string, request []byte) ([]byte, error) {
...
func (t *UDPv5) TalkRequestToID(id snode.ID, addr *net.UDPAddr, protocol string, request []byte) ([]byte, error) {
	req := &v5wire.TalkRequest{Protocol: protocol, Message: request}
...
func (t *UDPv5) RandomNodes() snode.Iterator {
	if t.tab.len() == 0 {
...
func (t *UDPv5) Lookup(target snode.ID) []*snode.Node {
...
func (t *UDPv5) lookupRandom() []*snode.Node {
	return t.newRandomLookup(t.closeCtx).run()
}

// lookupSelf looks up our own node ID.
// This is needed to satisfy the transport interface.
func (t *UDPv5) lookupSelf() []*snode.Node {
...
func (t *UDPv5) newLookup(ctx context.Context, target snode.ID) *lookup {
...
func (t *UDPv5) lookupWorker(destNode *node, target snode.ID) ([]*node, error) {
	var (
...
func lookupDistances(target, dest snode.ID) (dists []uint) {
	td := snode.LogDist(target, dest)
...
func (t *UDPv5) ping(n *snode.Node) (uint64, error) {
	req := &v5wire.Ping{snrSeq: t.localNode.Node().Seq()}
	resp := t.callToNode(n, v5wire.PongMsg, req)
	defer t.callDone(resp)

	select {
	case pong := <-resp.ch:
		return pong.(*v5wire.Pong).snrSeq, nil
	case err := <-resp.err:
		return 0, err
	}
}

// Requestsnr requests n's record.
func (t *UDPv5) Requestsnr(n *snode.Node) (*snode.Node, error) {
...
func (t *UDPv5) findnode(n *snode.Node, distances []uint) ([]*snode.Node, error) {
...
func (t *UDPv5) waitForNodes(c *callV5, distances []uint) ([]*snode.Node, error) {
	defer t.callDone(c)

	var (
		nodes           []*snode.Node
		seen            = make(map[snode.ID]struct{})
		received, total = 0, -1
	)
	for {
		select {
		case responseP := <-c.ch:
			...
			for _, record := range response.Nodes {
				node, err := t.verifyResponssnode(c, record, distances, seen)
				if err != nil {
...
// verifyResponssnode checks validity of a record in a NODES response.
func (t *UDPv5) verifyResponssnode(c *callV5, r *snr.Record, distances []uint, seen map[snode.ID]struct{}) (*snode.Node, error) {
	node, err := snode.New(t.validSchemes, r)
	if err != nil {
		return nil, err
	}
...
func (t *UDPv5) callToNode(n *snode.Node, responseType byte, req v5wire.Packet) *callV5 {
...
func (t *UDPv5) callToID(id snode.ID, addr *net.UDPAddr, responseType byte, req v5wire.Packet) *callV5 {
...
func (t *UDPv5) sendNextCall(id snode.ID) {
...
func (t *UDPv5) sendResponse(toID snode.ID, toAddr *net.UDPAddr, packet v5wire.Packet) error {
	_, err := t.send(toID, toAddr, packet, nil)
	return err
}

func (t *UDPv5) sendFromAnotherThread(toID snode.ID, toAddr *net.UDPAddr, packet v5wire.Packet) {
	select {
...
func (t *UDPv5) send(toID snode.ID, toAddr *net.UDPAddr, packet v5wire.Packet, c *v5wire.Whoareyou) (v5wire.Nonce, error) {
	addr := toAddr.String()
...
func (t *UDPv5) handleCallResponse(fromID snode.ID, fromAddr *net.UDPAddr, p v5wire.Packet) bool {
...
func (t *UDPv5) getNode(id snode.ID) *snode.Node {
...
func (t *UDPv5) handle(p v5wire.Packet, fromID snode.ID, fromAddr *net.UDPAddr) {
	switch p := p.(type) {
	case *v5wire.Unknown:
...
func (t *UDPv5) handleUnknown(p *v5wire.Unknown, fromID snode.ID, fromAddr *net.UDPAddr) {
...
func (t *UDPv5) handleWhoareyou(p *v5wire.Whoareyou, fromID snode.ID, fromAddr *net.UDPAddr) {
	...
	if c.node == nil {
		// Can't perform handshake because we don't have the snr.
		t.log.Debug("Can't handle "+p.Name(), "addr", fromAddr, "err", "call has no snr")
		c.err <- errors.New("remote wants handshake, but call has no snr")
		return
	}
...
func (t *UDPv5) matchWithCall(fromID snode.ID, nonce v5wire.Nonce) (*callV5, error) {
...
func (t *UDPv5) handlePing(p *v5wire.Ping, fromID snode.ID, fromAddr *net.UDPAddr) {
	...
	t.sendResponse(fromID, fromAddr, &v5wire.Pong{
		...
		snrSeq: t.localNode.Node().Seq(),
	})
}

// handleFindnode returns nodes to the requester.
func (t *UDPv5) handleFindnode(p *v5wire.Findnode, fromID snode.ID, fromAddr *net.UDPAddr) {
	nodes := t.collectTablsnodes(fromAddr.IP, p.Distances, findnodeResultLimit)
	for _, resp := range packNodes(p.ReqID, nodes) {
		t.sendResponse(fromID, fromAddr, resp)
	}
}

// collectTablsnodes creates a FINDNODE result set for the given distances.
func (t *UDPv5) collectTablsnodes(rip net.IP, distances []uint, limit int) []*snode.Node {
	var bn []*snode.Node
	...
	for _, dist := range distances {
		...
		for _, n := range t.tab.appendLivsnodes(dist, bn[:0]) {
			// Apply some pre-checks to avoid sending invalid nodes.
			// Note liveness is checked by appendLivsnodes.
...
func packNodes(reqid []byte, nodes []*snode.Node) []*v5wire.Nodes {
	if len(nodes) == 0 {
