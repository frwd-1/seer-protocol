...
// Package snr implements Seer Node Records as defined in EIP-778. A node record holds
...
//
// Package Snr supports the "secp256k1-keccak" identity scheme.
package snr
...
func (r *Record) Load(e Entry) error {
	i := sort.Search(len(r.pairs), func(i int) bool { return r.pairs[i].k >= e.SNRKey() })
	if i < len(r.pairs) && r.pairs[i].k == e.SNRKey() {
		if err := rlp.DecodeBytes(r.pairs[i].v, e); err != nil {
			return &KeyError{Key: e.SNRKey(), Err: err}
		}
		return nil
	}
	return &KeyError{Key: e.SNRKey(), Err: errNotFound}
}
...
func (r *Record) Set(e Entry) {
	...
	if err != nil {
		panic(fmt.Errorf("snr: can't encode %s: %v", e.SNRKey(), err))
	}
	r.invalidate()

	pairs := make([]pair, len(r.pairs))
	copy(pairs, r.pairs)
	i := sort.Search(len(pairs), func(i int) bool { return pairs[i].k >= e.SNRKey() })
	switch {
	case i < len(pairs) && pairs[i].k == e.SNRKey():
		// element is present at r.pairs[i]
		pairs[i].v = blob
	case i < len(r.pairs):
		// insert pair before i-th elem
		el := pair{e.SNRKey(), blob}
		pairs = append(pairs, pair{})
		copy(pairs[i+1:], pairs[i:])
		pairs[i] = el
	default:
		// element should be placed at the end of r.pairs
		pairs = append(pairs, pair{e.SNRKey(), blob})
	}
	r.pairs = pairs
}
...
func (r *Record) SetSig(s IdentityScheme, sig []byte) error {
	...
	case s == nil && sig != nil:
		panic("Snr: invalid call to SetSig with non-nil signature but nil scheme")
	case s != nil && sig == nil:
		panic("Snr: invalid call to SetSig with nil signature but non-nil scheme")
